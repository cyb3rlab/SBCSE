...
class RpfMitmMode(RPF_ModeHandler):
    def start_FakeMachine(self):
        if self.rpf_handler.rob_dt_data_dict[next(iter(self.rpf_handler.rob_dt_data_dict))][R.FLOOR] != self.rpf_handler.task_floor:
            self.fake_machine.move_to_elevator()
            while not self.rpf_handler.rob_dt_data_dict[next(iter(self.rpf_handler.rob_dt_data_dict))][R.ELV_PREPARATION] == RobotConfig.ELV_PREPARATION_Completed:
                pass
...
class FakeMachine:
    def __init__(self, handler, retry_interval=4, max_retries=6):
        ...
        self.states = {
            ...
            S.R1: self.simulate_crash_into_elevator,  # add
    ...
    def interlock(self):
        if self.fake_status == S.E0:
            ...
            if self.check_success(ELV.INTERLOCK_TRUE_SUCCESS):
                ...
                self.handler.send_B2R_command(target_floor=self.target_floor, command=R.CALLING, status=R.CALLING, rob_name=next(iter(self.handler.rob_status_dict)))
    ...
    def simulate_crash_into_elevator(self):
        # print("Simulating robot crashing into the elevator doors.")
        if self.check_success(R.GETTING_ON_SUCCESS):
            if self.check_success(R.GETTING_ON_COMPLETED):
                self.fake_status = S.COMPLETED  # Rob leaves the elevator and turns to close the elevator
        else:
    ...
    def completed(self):
        ...
        # self.init_rcp_status()  # Initialize after completion
