"""
The basic function is consistent with mqtt_communication_module/Scenario_msg_handler/Normal_mode/RPF_normal_mode.py, but added other fuzzing functions.
"""
import threading
import sys
...
class FRpfNormalMode2(RPF_ModeHandler):
    ...
    def start_RCPMachine(self, rob_name, stop_event):
        ...
        if not self.rpf_handler.rob_dt_data_dict[rob_name] or not self.robothandler.rob_dict[rob_name][RobotConfig.TASK]:
            return
    ...
    def handle_task(self, stop_event):
        self.elvhandler = ElvHandler()
        self.robothandler = RobotHandler()
        threads = []
        for rob_name, rob_attributes in self.robothandler.rob_dict.items():
            ...
            thread = threading.Thread(target=self.start_RCPMachine, args=(rob_name, stop_event,))
